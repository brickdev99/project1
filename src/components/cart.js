      ...
      var datas = this.props.cartData.map((data, i) => {
        ...
        return (
          <div className="row cart-info">
            <div
              className="col-sm-1 cur"
              onClick={() => this.props.deleteItem(i)}
              onMouseOut={() => this.cancelItem1(i)}
              onMouseOver={() => this.cancelItem(i)}
            >
              &times;
            </div>
            <div className="col-sm-3">
              <img style={{ height: "90px" }} src={data.image} />
            </div>
            <div
              className={"col-sm-3" + " " + "cart-name" + i + " " + "cart-name"}
            >
              {data.name}
            </div>
            <div
    ...
    return (
      <div>
        <div className="cart-header">
          <div onClick={this.closeDiv} className="cart-button-close">
            &times;
          </div>
          <div
          ...
          <h1 className="text-center">
            <span class="glyphicon">
              &#xe116;
              <span className="cart-value">{this.props.cartData.length}</span>
            </span>
          </h1>
          <div onClick={this.openDiv} className="cart-button">
            <span class="glyphicon">
              &#xe116;
              <span className="cart-value">{this.props.cartData.length}</span>
            </span>
          </div>
          <div className="cart-data">{datas}</div>
